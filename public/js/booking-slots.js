...
class BookingSlotManager {
    ...
    async fetchSlots(date) {
        ...
        try {
            ...
            // Get the clinic location (if available in form, otherwise use default 'seremban')
            const clinicInput = document.getElementById('clinic_location');
            const clinicLocation = clinicInput ? clinicInput.value : 'seremban';

            const response = await fetch(
                `${this.slotApiUrl}?date=${date}&service_id=${serviceId}&clinic_location=${clinicLocation}`
            );
